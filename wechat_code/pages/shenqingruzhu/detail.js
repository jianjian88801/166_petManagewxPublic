...
goodid: this.data.detailList.id,
    goodname:  this.data.detailList.jiuzhuzhanmingcheng,
...
const {
data
} = await detail("shenqingruzhu",id)
this.setData({
payAuth:utils.isAuthFront('shenqingruzhu','支付')
})
...
const resUpdate = await update('shenqingruzhu', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','shenqingruzhu');
...
async reply() {
const detailList = this.data.detailList
const res = await update("shenqingruzhu", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
