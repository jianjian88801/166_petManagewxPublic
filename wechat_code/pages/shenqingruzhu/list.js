...
if(this.data.jiuzhuzhanmingcheng){
    searchForm['jiuzhuzhanmingcheng'] = '%' + this.data.jiuzhuzhanmingcheng + '%'
}


let res = {};
if(this.data.userid) {
res = await page(`shenqingruzhu`, searchForm);
} else {
res = await list(`shenqingruzhu`, searchForm);
}
...
let targetName="jiuzhuzhanmingcheng"
...
wx.navigateTo({
url: `/pages/shenqingruzhu/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("shenqingruzhu",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/shenqingruzhu/update-and-add?id=${id}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
url: `/pages/shenqingruzhu/detail`
})
...
isAuthObj['addAuth']   = utils.isAuth("shenqingruzhu", "新增")
isAuthObj['delAuth']  = utils.isAuth("shenqingruzhu", "删除")
isAuthObj['editAuth']   =utils.isAuth("shenqingruzhu", "修改")
}else{
isAuthObj['addAuth']   = utils.isAuthFront("shenqingruzhu", "新增")
isAuthObj['delAuth']  = utils.isAuthFront("shenqingruzhu", "删除")
isAuthObj['editAuth']   =utils.isAuthFront("shenqingruzhu", "修改")
}
//前后台权限判断
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("shenqingruzhu",obj)
    }else{
         resList= await list("shenqingruzhu",obj)
    }
...
.map(item => {
item.tupian? item.tupian = item.tupian.split(",")[0].replace('upload','flie'):""
return item;
});
